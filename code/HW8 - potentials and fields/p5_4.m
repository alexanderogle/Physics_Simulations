...
%plot the electric field of the capacitors
dim_Vo = size(Vo);
[X,Y] = meshgrid(-x_d:dx:x_d);
[U,V] = gradient(Vn);
%quiver(X,Y,U,V)

mesh(x,y,Vn)
title('Electric Potential of Parallel Plate Capacitor, d = 0.1')
xlabel('x position')
ylabel('y position')
zlabel('Potential')
spacing = scale*x_d;
disp('The capacitor spacing is ')
disp(spacing)
